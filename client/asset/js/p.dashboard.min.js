...
var mapCover = $('#dashboard-map-cover')
...
var searchInput = $('#txt-dashboard-search')
var searchInputGoogle = 'txt-dashboard-search-temp'
var googleAutoComplete = 'body > .pac-container:last-child'
...
function initMarker(pin){
  ...
  google.maps.event.addListener(marker, 'click', function(e){
    ...
    detailClose.unbind().on('click', function(){
      ...
      mapCover.addClass('hidden')
      canClickMenuButton = true
...
function initMarkTrigger(){
  ...
  var searchInputTemp = document.getElementById(searchInputGoogle)
  var searchBox = new google.maps.places.SearchBox(searchInputTemp)
  mapincGoogleMap[mapName].controls[google.maps.ControlPosition.TOP_LEFT].push(searchInputTemp)

  searchInput.unbind().on('keyup', function(e){
    if(e.which == 13 || e.which == 38 || e.which == 40){
      e.preventDefault()
      if(e.which == 13) searchAutoComplete.removeClass('open').html('')
    } else {
      $(`#${searchInputGoogle}`).val($(this).val())
      if(_.isEmpty($(this).val())){
        $(this).val('')
        google.maps.event.trigger(searchInputTemp, 'blur')
        searchAutoComplete.removeClass('open').html('')
      } else {
        google.maps.event.trigger(searchInputTemp, 'focus')
      }
    }
    ...
    if(e.which == 13){
      e.preventDefault()
      google.maps.event.trigger(searchInputTemp, 'focus')
      google.maps.event.trigger(searchInputTemp, 'keydown', { keyCode: 13 })
    } else if(e.which == 38 || e.which == 40){
      e.preventDefault()
      google.maps.event.trigger(searchInputTemp, 'keydown', { keyCode: e.which })
      $(this).val($(`#${searchInputGoogle}`).val())
    }
    searchAutoComplete.html($(googleAutoComplete).html()).addClass('open')
  })
  ...
  searchBox.addListener('places_changed', function(){
    ...
    searchMarkers.forEach(function(marker){ marker.setMap(null) })
    searchMarkers = []
